...
def GammaLine_Counting(energies, detector, measurement, meas_ID, energy_filter, energy_resolution_par, cuts, OutPath):
    ...
    #prepare histogram
    xmin_99_103, xmax_99_103 = 180, 194 #95, 107
    ...
    R =  0.0203/0.0195
    mu_99_guess, mu_103_guess, mu_small_guess = 183.5, 191, 187.5 #99, 103, mu_s
    ...
    a_99_guess, a_103_guess, a_small_guess = max(hist_peak)*R, max(hist_peak), max(hist_peak)*0.1
    ...
    try:
        ...
        #compute chi sq of fit
        chi_sq, p_value, residuals, dof = ChiSqCalc(bins_centres_peak, hist_peak, np.sqrt(hist_peak), peak_fitting.gauss_cdf, coeff)
        print("r chi sq: ", chi_sq/dof)
...
def gauss_count(a,mu,sigma, err_A, bin_width):
    ...
    integral_list = quad(peak_fitting.gauss, 0,200, args=(mu,sigma,a)) #0,120
